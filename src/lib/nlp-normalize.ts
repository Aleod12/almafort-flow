...
const SLANG: Array<[RegExp, string]> = [
  [/\bканд(ер|ей|юк)\w*/gi, "кондиционер"],
  [/\bконде[йя]\w*/gi, "кондиционер"],
  [/\bсплит[- ]?систем\w*/gi, "кондиционер"],
  [/\bсендвич\w*/gi, "сэндвич-панель"],
  [/\bсэндвич(?!-панел)\w*/gi, "сэндвич-панель"],
  [/\bсендвич[- ]?панел\w*/gi, "сэндвич-панель"],
  [/\bкрепс\w*/gi, "КРЕПСС крепёж для сэндвич-панелей"],
  [/\bчопик\w*|\bзатычк\w*|\bпробк\w*/gi, "заглушка"],
  [/\bкопыт\w*|\bбашмак\w*|\bпятк\w*|\bножк\w*/gi, "опора"],
  [/\bкраб\w*|\bклипс\w*/gi, "кляймер"],
  [/\bтерраск\w*|\bдек(инг)?\b/gi, "террасная доска ДПК"],
  [/\bпрофтруб\w*|\bпрофил[ья]?труб\w*/gi, "профильная труба"],
  [/\bгипрок\w*|\bгкл\b/gi, "гипсокартон"],
  [/\bкрепёж|\bкрепеж/gi, "крепёж"],
];
...
const MASS_UNITS: Array<[RegExp, number]> = [
  [/(\d[\d\s.,]*)\s*(?:т(?:онн\w*)?)\b/i, 1000],
  [/(\d[\d\s.,]*)\s*(?:кг|килограмм\w*)/i, 1],
  [/(\d[\d\s.,]*)\s*(?:г(?:рамм\w*)?)\b/i, 0.001],
];
...
export function normalizeQuery(raw: string): NormalizedQuery {
  ...
  const thick = text.match(/толщин\w*\s*(?:панели\s*)?(\d{2,4})|(\d{2,4})\s*мм/i);
...
const INJECTION = [
  ...
  /покажи|выведи|раскрой/i.source && /(промпт|prompt|инструкц\w+, которым)/i,
...
export function clarificationQuestions(n: NormalizedQuery): string[] {
  const loadTask =
    /закреп|повес|подвес|монтаж|крепёж|крепеж|нагруз|станок|оборудован|тяжел/i.test(n.text);
  if (!loadTask) return [];
